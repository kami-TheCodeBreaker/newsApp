...
const NewsItem=(props)=> {
  const checkIfImageExists=async(url)=> {
    ...
    if (!checkIfImageExists(imageUrl)) imageUrl = noImage;
    return (
      <div
        className="card shadow-xl rounded w-fit px-2 py-2 my-9 font-ubuntu	"
        style={{ position: "relative" }}
      >
        <div
          className="badge bg-orange-700 rounded text-white px-3 py-2"
        ...
        <div className="text px-2">
          ...
          <p className="w-image my-3">
            {description
              ...
              : "Taxiarchis Fountas hat D.C. United am Sams"}{" "}
            ...
          </p>
          <span>
            By{" "}
            <span className=" text-md underline hover:text-blue-700 hover:cursor-pointer">
              {author
                ? author.length < 20
                  ? author
                  : author.slice(0, 20) + " ..."
